...
@section('content')
    ...
    <h2 class="title text-center">Edit {{ ucfirst($item->name) }} Item</h2>

    <form class="info" method="post" action="{{ route('dashboard.update.item', ['item_id' => $item->id]) }}"
        ...
        <div class="row">
            ...
            <div class="col-sm-12 col-md-4">
                <img src="{{ asset('/') . $item->image_url }}" width="50%" alt="No Image">
            </div>
            <div class="col-sm-12">
                ...
                <textarea name="description" id="description" rows="3"
                    class="form-control">{{ $item->description->description ?? '' }}</textarea>
                @error('description')
                    <small class="text-danger">{{ $message }}</small>
                @enderror
            </div>
